...
export class FriendService {
  static async sendFriendRequest(senderId: string, receiverId: string) {
    try {
      if (senderId === receiverId) throw createError('You cannot send a request to yourself', 400);
  ...
  static async respondToRequest(requestId: string, userId: string, action: 'accept' | 'reject' | 'block') {
    try {
      const requestRepo = AppDataSource.getRepository(FriendRequest);
